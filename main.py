...
parser.add_argument("-proc", "--processor", required=True,
                    help="Choose the kind of processor to simulate.",
                    choices=['simple', 'pipelined', 'scheduled'],
                    dest='processor_type'
                    )

parser.add_argument("-pred", "--predictor", required=True,
...
cpu = processor(instructions, symbols, prediction_method=args.prediction_method, debug=args.debug)
...
test_result = click.style('PASSED', fg='green') if tests[args.filename.name](
