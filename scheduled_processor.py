...
class Instruction:
    def __str__(self):
        if self.opcode in ["lw", "sw"]:
            if self.opcode == "lw":
                return f"{self.opcode}, loading from $r{self.target_register} to address $r{self.source_reg1} + {self.immediate}"
            elif self.opcode == "sw":
                return f"{self.opcode}, storing what is in ${self.source_reg1} from address $r{self.source_reg2} + {self.immediate}"
            else:
                raise RuntimeError("IDK HOW TO PRINT STORES EHEHEH")
        else:
            return f"{self.opcode}, {self.target_register} <- {self.source_reg1} op {self.source_reg2}/{self.immediate}"
...
class ReorderBuffer:
    def __str__(self):
        ...
        sslice = (
            self.entries[:18]
            if self.commit_pointer < 9
            else self.entries[self.commit_pointer - 9 : self.commit_pointer + 9]
        )
        sslice = self.entries
        for i, e in enumerate(sslice):

            if e is None:
                data.append(["empty"] * len(header))
            else:
                pointer = "->" if e.id == self.commit_pointer else ""
                data.append(
                    [pointer, e.id, e.opcode, e.destination, e.value, e.pc, e.done]
                )
    ...
    def add(self, instruction: Instruction, pc: int) -> Tuple[ReorderBufferEntry, int]:
        ...
        destination = (
            32
            if instruction.is_store() or instruction.is_branch()
            else instruction.target_register
        )  # dummy value if needed

        entry = ReorderBufferEntry(
        ...
        if instruction.is_branch():
            entry.pc = instruction.immediate
        else:
            entry.pc = instruction.fetched_at_pc + 1
...
class RegisterFile:
    ...
    def commit(self, rob_entry: ReorderBufferEntry, rob_commit_pointer: int):
        if not rob_entry.done:
            raise ValueError("BAD ROB Entry")
    ...
    def read_registers(self, instruction: Instruction, ROB: ReorderBuffer):
        ...
        if s2 is None:
            if instruction.is_branch():
                raise RuntimeError("this is fucked")
...
class LoadStoreQueueEntry:
    ...
    def __init__(
        self, opcode, dest_tag, target_address, offset, tag_base, base, tag_value, value
    ):
        self.opcode = opcode
    ...
    def is_ready(self, queue):
        ...
        if self.opcode == "lw":
            ...
            prev_stores = [
                e
                for e in queue[: self.id]
                if e is not None
                and e.target_address == self.target_address
                and e.opcode == "sw"
            ]
            ...
            # todo otherwise we wait for an address to be forwarded to us
                ...
                return (
                    self.target_address is not None
...
class LoadStoreQueue:
    ...
    def forward_store(self, lsq_entry_to_commit: LoadStoreQueueEntry, value):
        next_entries = [
            e
            for e in self.entries[lsq_entry_to_commit.id :]
            if e is not None and e.opcode == "lw"
        ]
    ...
    def capture(self, tag, value):
        ...
        for e in [e for e in self.entries if e is not None]:
            if (
                e.tag_base is not None
                and e.tag_value is not None
                and e.tag_base == e.tag_value
            ):
                raise RuntimeError("Tags match, reimplement")

            if e.opcode == "sw" and e.tag_value == tag:
                e.value = value
            if e.tag_base == tag:
                self.entries[e.id].base = value
    ...
    def add(
        ...
        if instruction.opcode == "sw":
            ...
            if (
                instruction.target_register is not None
                and instruction.source_reg1 is None
                or instruction.source_reg2 is None
            ):
    ...
    def hydrate_loads(self):
        ...
        loads = [
            ...
            if e is not None and e.opcode == "lw" and e.id >= self.commit_pointer
        ]

        for load in loads:
            prev_stores = [
                e for e in self.entries[: load.id] if e is not None and e.opcode == "sw"
            ]
    ...
    def get_next_ready(self):
        ...
        ready_entry_ixs = [
            ...
            if e is not None and e.is_ready(self.entries)
        ]

        if not ready_entry_ixs:
            return None
...
class ReservationStation:
    def __str__(self):
        ...
        for i, e in enumerate(self.entries[:6]):
            ...
            if e is None:
                ...
                data.append(
                    [e.opcode, e.dest_tag, e.tag1, e.tag2, e.val1, e.val2, e.is_ready()]
                )
    ...
    def add(
        ...
        if instruction.is_jump():
            ...
            entry = ReservationStationEntry(
                instruction.opcode, rob_pointer, t1, t2, v1, v2
            )
    ...
    def get_next_ready(self):
        ready_entry_ixs = [
            i for i, e in enumerate(self.entries) if e is not None and e.is_ready()
        ]
...
class Decoder:
    ...
    def decode(self, instruction_string: str, fetched_at_pc: int) -> Instruction:
        ...
        target_register, source_registers, immediate = self.fetch_register_names(
            opcode, operand_str
        )
    ...
    def fetch_register_names(self, opcode, operand_str):
        ...
        if opcode in ["add", "sub", "mul", "mod", "div", "imul", "idiv"]:
            ...
            operands = self.parse_operands(
                operand_str, "^(?P<rd>\$\w*) (?P<rs>\$\w*) (?P<rt>\$\w*)$"
            )
            ...
            operands = self.parse_operands(
                operand_str, "^(?P<rt>\$\w*) (?P<rs>\$\w*) (?P<imm>-*\d+)$"
            )
            ...
            operands = self.parse_operands(
                operand_str, "^(?P<rt>\$\w*) *(?P<imm>\w+)\((?P<rs>\$\w*|\d*)\)$"
            )
            ...
            operands = self.parse_operands(
                operand_str, "^(?P<rs>\$\w*) (?P<rt>\$\w*) (?P<imm>-*\d+|\$*\w*)$"
            )
            source_registers[0] = operands["rs"]
            source_registers[1] = operands["rt"]
            immediate = operands["imm"]
    ...
    def execute(self, rs_entry: ReservationStationEntry):
        ...
        if opcode == "add":
            ...
            if opcode == "blt":
                ...
                raise RuntimeError(f"{opcode} not implemented")

        else:
            raise RuntimeError(
                f"You are not supposed to perform ALU operation on operation {opcode}"
            )
...
class ScheduledProcessor(Processor):
    ...
    def fetch(self):
        ...
        if self.PC > len(self.program) or self.PC < 0:
            raise RuntimeError(
                f"PC out of bounds. PC={self.PC} for program of length {len(self.program)}"
            )
        ...
        return [
            ("push", "decode_queue", (instruction_string, self.PC)),
            ("set", "PC", predicted_pc),
        ]
    ...
    def issue(self):
        ...
        if (len(self.issue_queue) > 0) and self.rob.is_available():
            ...
            if instruction.is_mem():
                lsq_entry = self.lsq.add(
                    instruction, rob_entry_pointer, self.rf, self.rob
                )
            else:
                rs_entry = self.rs.add(
                    instruction, rob_entry_pointer, self.rf, self.rob
                )
    ...
    def mem(self):
        ...
        if len(self.mem_queue) > 0:
            ...
            if lsq_entry.opcode == "lw":
                ...
                raise ValueError("Something is really wrong with the mem component")
        ...
        if self.rob.can_commit():
            ...
            if Decoder.is_mem(rob_entry_to_commit.opcode):
                ...
                if rob_entry_to_commit.opcode == "sw":
                    # perform the store
                    self.MEM[
                        lsq_entry_to_commit.target_address
                    ] = rob_entry_to_commit.value

                    # notify all the loads with matching address of the value, this allows them to dispatch
                    self.lsq.forward_store(
                        lsq_entry_to_commit, rob_entry_to_commit.value
                    )
